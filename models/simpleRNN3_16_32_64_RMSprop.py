...
def simpleRNN3_16_32_64_RMSprop(x_train, y_train, x_validation, y_validation, x_test, y_test, spec_count, dataset, sc):
    model = Sequential()

    model.add(SimpleRNN(16, input_shape=(x_train.shape[1], spec_count), return_sequences=True))
    model.add(SimpleRNN(32, input_shape=(x_train.shape[1], spec_count), return_sequences=True))
    model.add(SimpleRNN(64, input_shape=(x_train.shape[1], spec_count)))
    model.add(Dense(1))
